...
object ActorsAndThreads2 extends App {
  ...
  val as = ActorSystem("as")
  // create 100 actors
  val actors = 0 to 100 map (i => as.actorOf(Props(new PrintActor(i)), "p"+i))
  // send each actor 10 msgs in parallel
  (0 to 1000).par.foreach (i => actors(i % 100) ! "MSG")
}
...
object UnhandledMessage extends App {
  ...
  class Listener extends Actor {
    def receive = {
      case d: UnhandledMessage => println(s"Listener: $d")
    }
  }
  ...
  val a = as.actorOf(Props[PrintActor], "a")
  a ! 12
}
