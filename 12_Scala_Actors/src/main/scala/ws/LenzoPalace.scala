...
object LenzoPalace {
  ...
  // Boss -> Waiter
  case class Total(boss: ActorRef)
  ...
  class Waiter(cook: ActorRef) extends Actor {
    ...
    def receive = {
      case Order(food, drink, customerRef) =>
        total += (if (food == Döner) 8 else 9)
        total += 3
        cook ! FoodOrder(food, customerRef)
        customerRef ! Glass(drink)

      case Total(chef) => chef ! total
    }
  }

  class Cook extends Actor {
    def receive = {
      case FoodOrder(f, customer) => customer ! Plate(f)
    }
  }

  class Boss(waiter: ActorRef) extends Actor {
    waiter ! Total(self)

    def receive = {
      case num => println(s"My Money: $num")
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    as.actorOf(Props(new Boss(waiter)))
