...
class RxIrcBot(out: OutputStreamWriter, in: ConnectableObservable[String]) {
  ...
  def init(): Unit = {
    ...
    Thread.sleep(5000) // wait until user is accepted
    sendCmd("JOIN", chan)

    // Play PING -> PONG (keep alive)
    in.filter(cmd => isPing(cmd)).subscribe(cmd => sendPong(cmd))

    // Log all messages
    if (logMessages) in.subscribe(cmd => println("< " + cmd), exc => println(exc))

    // Handle messages
    val messages =  in.filter(isPrivMsg).map(senderMsg)
    messages.filter(msg => msg.startsWith(":")).subscribe(cmd => handleCommand(cmd))
  ...
  def handleCommand(msg: String): Unit = {
    msg.split(" ",2) match {
      case Array(":time") => sendMsg(s"Now it is ${new Date()}")
      case Array(":rot13", txt) => sendMsg(s"Your message in rot13: ${rot13(txt)}")
      case _ => sendMsg(s"I can't handle: $msg")
    }
  }
